...
| VDSina, прежний GitHub Pages proxy | `auto-team.pro` | nginx proxy | Перенос не завершён: старый Pages job `deploy` остаётся `queued` с 12 августа и блокирует новые runs через `concurrency: pages` |
