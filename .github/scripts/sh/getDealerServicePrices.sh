...
# Если DEALER_SERVICE_PRICE_CSV_URL не установлен, пытаемся получить его из .env
if [ -z "$CSV_URL" ] && [ -f .env ]; then
    export CSV_URL=$(grep '^DEALER_SERVICE_PRICE_CSV_URL=' .env | awk -F'=' '{print substr($0, index($0,$2))}' | sed 's/^"//; s/"$//')
fi

# Проверяем, что CSV_URL установлен
if [[ ! "$CSV_URL" =~ ^https?:// ]]; then
    echo "Error: DEALER_SERVICE_PRICE_CSV_URL is not found or empty"
...
if [ -z "$QUERY_STRING" ] && [ -f .env ]; then
    export QUERY_STRING=$(grep '^DEALER_SERVICE_PRICE_QUERY_STRING=' .env | cut -d '=' -f 2- | sed 's/^"//; s/"$//')
fi

# Проверяем, что QUERY_STRING установлен
if [ -z "$QUERY_STRING" ]; then
    echo "Error: DEALER_SERVICE_PRICE_QUERY_STRING is not found"
    # Если IGNORE_ERRORS=1, не считаем это ошибкой
    if [ "$IGNORE_ERRORS" = "1" ]; then
        echo "IGNORE_ERRORS=1: Пропускаем ошибку и продолжаем выполнение"
        exit 0
...
if [ -z "$KEY_MAPPING" ] && [ -f .env ]; then
    export KEY_MAPPING=$(grep '^DEALER_SERVICE_PRICE_KEY_MAPPING=' .env | awk -F'=' '{print substr($0, index($0,$2))}' | sed 's/^"//; s/"$//')
fi

# Проверяем, что KEY_MAPPING установлен
if [ -z "$KEY_MAPPING" ]; then
    echo "Error: DEALER_SERVICE_PRICE_KEY_MAPPING is not found"
    # Если IGNORE_ERRORS=1, не считаем это ошибкой
    if [ "$IGNORE_ERRORS" = "1" ]; then
        echo "IGNORE_ERRORS=1: Пропускаем ошибку и продолжаем выполнение"
        exit 0
...
export KEY_COLUMN=""
export OUTPUT_PATHS="./src/data/site/dealer-service-price.json"
