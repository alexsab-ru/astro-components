...
function AvtoInfoResult() {
	...
	return ( 
		<>
			<h3 className="text-3xl font-bold mb-10">Расчетная цена {avtoInfo?.brand.name} {avtoInfo?.model.name} — <span className="text-nowrap">{avtoInfo?.year} г.в.</span></h3>
			<div className="lg:w-1/3 bg-gray-100 p-6">
				<h3 className="text-2xl font-bold mb-6">Выкуп</h3>
				{dealerPrice !== 0 ? (
					<div className="mb-6">
						<h3 className="text-3xl font-bold text-nowrap">{ dealerPrice.toLocaleString('ru-RU') } ₽</h3>
			...
			<div className="flex flex-wrap justify-between items-center gap-5 text-gray-800 font-medium py-10 px-6 border-b border-b-gray-300 mb-12">
				...
				{selfSale !== 0 ? (
						<p className="font-bold !mb-2 text-nowrap">{ selfSale.toLocaleString('ru-RU') } ₽</p>
					) : (
