...
function AvtoInfoList() {
	...
	return (
		<div className="w-full lg:w-1/3 lg:pr-10 py-8 lg:py-16 px-5 bg-gray-100">
			<h3 className="text-xl md:text-2xl font-weight-bold mb-6 md:mb-8">Ваш автомобиль</h3>
				...
				<ul className="flex flex-col">
					...
					{Object.keys(avtoInfo).map(key => rusKey(key) && useTranslit(typeof avtoInfo[key] === 'object' ? avtoInfo[key]?.name : avtoInfo[key]) && (
						...
						>
							...
							{key === 'engineVolume' ? (
								<span className="font-weight-bold">{ useTranslit(typeof avtoInfo[key] === 'object' ? round(avtoInfo[key]?.name) : round(avtoInfo[key])) }</span>
							) : (
								<span className="font-weight-bold">{ useTranslit(typeof avtoInfo[key] === 'object' ? avtoInfo[key]?.name : avtoInfo[key]) }</span>
							)}
						</li>
					) )}
					...
					{Number(mileageState) !== 0 && (
						<li className="flex justify-between order-last mt-5">
							<span>Пробег</span>
							<span className="grow border-b border-dotted mx-2 mb-2"></span>
							<span className="font-weight-bold">{Number(mileageState).toLocaleString('ru-RU')} км</span>
						</li>
					)}
